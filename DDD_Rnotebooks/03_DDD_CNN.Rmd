...
set.seed(113)
device = "cuda" # change if you want to compute GPUs "cpu"
...
cbvl_f<-"phylogeny-DDD2-nt-10000-la0-0-50-mu-0-50-k-20-400-age-1-ddmod-10-cblv.rds"
...
if( file.exists(paste(path_ddd,cbvl_f,sep=""))==FALSE){
start_time <- Sys.time()
...
saveRDS(cblv,(paste(path_ddd,cbvl_f,sep="")))
}else{
  print(paste("Tree from model: ",model,", already generated. Reading file",sep=""))
  cblv <- readRDS( (paste(path_ddd,cbvl_f,sep="")))
}
```
...
while (epoch < n_epochs & trigger < patience) {
  ...
  if (current_loss< best_loss){
    ...
    torch_save(cnn, paste( "data_DDD/models_DDD/04_CNN","lay",n_layer,"hid",n_hidden,sep="-"))
    best_epoch<-epoch
...
# Plot the loss curve
png("data_DDD/plots/loss_cnn_norm.png")
...
result$timemin <- as.numeric(cnn_time)
result$unit <- units(cnn_time)
write.csv(result, file = "data_DDD/results/cnn_norm.csv", row.names = FALSE)
# Print the resul
print(result)
...
cnn<-torch_load( paste( "data_DDD/models_DDD/04_CNN","lay",n_layer,"hid",n_hidden,sep="-"))
...
png("data_DDD/plots/cnn_estimated.png", width = 480*3*factor, height = 480*factor,res=dpi)
par(mfrow = c(1, 4))
...
write.csv(mse_l, file = "data_DDD/results/cnn_msel_norm.csv", row.names = FALSE)
...
for (i in 1:length(h)) {
  ...
  png(paste("data_DDD/plots/NRMSE_cnn_nomr",names(h)[i],".png",sep=""), 
