...
cnnltt_timepdl<-end_time - start_time
...
# Plot the loss curve
png("data_PLD/plots/loss_cnnlttcorr_normpld.png")
...
result$timemin <- as.numeric(cnnltt_timepdl)
result$unit <- units(cnnltt_timepdl)
write.csv(result, file = "data_PLD/results/cnnltt_16_norm.csv", row.names = FALSE)
# Print the result
print(result)
...
png("data_PLD/plots/cnnltt_estimates.png", width = 480*3*factor, height = 480*factor,res=dpi)
par(mfrow = c(1, 4))
...
write.csv(mse_l, file = "data_PLD/results/cnnltt_norm.csv", row.names = FALSE)
...
for( i in 1:length(true)){
  ...
  for (j in names(grouped_indices)) {
    ...
    nrmse <- sqrt(mean((true[[i]][test_indices][tree_indices]-nn.pred[[i]][tree_indices])^2))/abs(mean(true[[i]][test_indices][tree_indices]))
...
for (i in 1:length(h)) {
  ...
  png(paste("data_PLD/plots/NRMSE_cnnlttcorrnomr",names(h)[i],".png",sep=""), 
  ...
  plot(x = as.integer(names(h[[i]])),
       ...
       ylab = "NRMSE",
       main = paste("NRMSE -", names(h)[i]),ylim=c(0,5))
  dev.off()
}
